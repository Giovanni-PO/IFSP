...
const EstoqueLocal = () => {
  ...
  const carregarEstoqueLocal = async () => {
    try {
      ...
      const salaAtual = String(codigo)
        .trim()
        .toUpperCase();

      const response = await fetch(
        `http://localhost:3000/locais/${encodeURIComponent(salaAtual)}/itens`
      );

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.erro || 'Erro ao carregar itens');
      }

      setItens(Array.isArray(data) ? data : []);
      setItensFiltrados(Array.isArray(data) ? data : []);
  ...
  const buscarItemExterno = async () => {
    ...
    try {
      const codigoExterno = novoItem.codigo
        .trim()
        .toUpperCase();

      const resp = await fetch(
        `http://localhost:3000/itens-externos/${encodeURIComponent(codigoExterno)}`
      );

      const data = await resp.json();

      if (!resp.ok) {
        alert(data.erro || "Item externo não encontrado");
        return;
      }
  ...
  const adicionarItem = async () => {
    try {
      ...
      if (!novoItem.numero && !novoItem.codigo) {
        alert("Digite o número do patrimônio ou código");
        return;
      }
  ...
  const deletarItem = async id => {
    ...
    try {
      ...
      if (!resp.ok) {
        alert(data.erro || "Erro ao deletar item");
        return;
      }
